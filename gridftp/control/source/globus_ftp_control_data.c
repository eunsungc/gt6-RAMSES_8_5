...
#define _RAMSES_DEBUG_
            // 1. Find the device
            sprintf(buf, "%s%s%s", "df ", ramses_log.file, " | tail -n 2 | head -n 1 | awk '{print $1}'");
            ...
            sprintf(buf, "%s%s%s", "iostat 1 2 -x ", devname, " | tail -n 2 | head -n 1"); // run iostat twice to measure throughput
#ifdef _RAMSES_DEBUG_
            printf("buf = %s\n", buf);
#endif
            fp = popen(buf, "r");
