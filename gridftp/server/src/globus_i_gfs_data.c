...
response_exit:
    ...
    if(op->cached_res == GLOBUS_SUCCESS)
    {
        ...
        if(!op->data_handle->http_handle && op->data_handle->is_mine)
        {
            ...
            //if (op->session_handle->udt_data_channel_inuse)
    ...
    void *                              user_arg)
{
	...
	case GLOBUS_GFS_EVENT_RANGES_RECVD:
        ...
        ramses_log.host = bounce_info->op->data_handle->session_handle->client_ip ? bounce_info->op->data_handle->session_handle->client_ip : "0.0.0.0";
